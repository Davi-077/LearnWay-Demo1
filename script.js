...
studyTypeEl?.addEventListener("change",()=>{
  ["school-fields","college-fields","course-fields"].forEach(id=>$(id)?.classList.add("hidden"));
  if(studyTypeEl.value==="escola") $("school-fields")?.classList.remove("hidden");
  if(studyTypeEl.value==="faculdade") $("college-fields")?.classList.remove("hidden");
  if(studyTypeEl.value==="curso") $("course-fields")?.classList.remove("hidden");
});
...
$("start-button").onclick = () => {
  ...
  if(!studyType){
    alert("Conte o que você estuda atualmente.");
    return;
  }
