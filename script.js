...
const studyTypeEl=$("study-type");

function showOnlyStudyFields(type){
  ["school-fields","college-fields","course-fields"].forEach(id=>{
    $(id)?.classList.add("hidden");
  });

  if(type==="escola") $("school-fields")?.classList.remove("hidden");
  if(type==="faculdade") $("college-fields")?.classList.remove("hidden");
  if(type==="curso") $("course-fields")?.classList.remove("hidden");
}

studyTypeEl?.addEventListener("change",()=>{
  showOnlyStudyFields(studyTypeEl.value);
});
...
$("start-button").onclick = () => {
  ...
  const studyType=$("study-type").value;

  if(!studyType){
    alert("Selecione o que você estuda atualmente.");
    return;
  }

  const education={type:studyType};

  if(studyType==="escola"){
    education.grade=$("school-grade").value;
    education.className=$("school-class").value.trim();
    if(!education.grade){
      alert("Selecione seu ano/série escolar.");
      return;
    }
  }

  if(studyType==="faculdade"){
    education.course=$("college-course").value.trim();
    education.period=$("college-period").value.trim();
    if(!education.course || !education.period){
      alert("Informe seu curso e período/semestre.");
      return;
    }
  }

  if(studyType==="curso"){
    education.course=$("course-name").value.trim();
    education.level=$("course-level").value.trim();
    if(!education.course){
      alert("Informe qual curso você faz.");
      return;
    }
  }
...
function renderExamFiles(){
  ...
  if(!examSelectedFiles.length){
    box.innerHTML='<div class="empty-mini">Nenhuma foto ou arquivo adicionado ainda.</div>';
    return;
  }
...
document.addEventListener("click",e=>{
  ...
  if(e.target.closest("#take-exam-photo")){
    $("exam-camera")?.click();
  }
  ...
  if(action){
    ...
    const typedContent=$("exam-content")?.value.trim() || "";
    if(!typedContent && !examSelectedFiles.length){
      alert("Conte o que vai cair na prova, tire uma foto ou adicione um arquivo.");
      return;
    }
    const names={
    ...
    if($("exam-result-text")){
      const typedContent=$("exam-content")?.value.trim() || "";
      const parts=[];
      if(typedContent) parts.push("conteúdo escrito");
      if(examSelectedFiles.length) parts.push(`${examSelectedFiles.length} foto/arquivo${examSelectedFiles.length>1?"s":""}`);
      $("exam-result-text").textContent=
        `${subject}: ${parts.join(" + ")} preparado(s). Na versão com IA, o Learn Way usará essas informações para executar esta ação.`;
    }
...
$("exam-camera")?.addEventListener("change",e=>{
  const newFiles=[...e.target.files];
  examSelectedFiles=[...examSelectedFiles,...newFiles];
  renderExamFiles();
  e.target.value="";
});
